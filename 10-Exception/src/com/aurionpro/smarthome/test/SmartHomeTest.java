...
public class SmartHomeTest {
		...
		while (true) {
			...
			try {
				...
				switch (choice) {
					...
					case 2:
						controlSpecificDevice(devices, deviceNames, scanner);
						break;
	...
	private static void controlSpecificDevice(IControllable[] devices, String[] deviceNames, Scanner scanner) {
		while (true) {
					...
					switch (action) {
						...
						case 2:
							if (!device.isDeviceOn()) {
								System.out.println("❌ " + deviceName + " is OFF. Please turn it ON before setting the mode.");
							} else {
